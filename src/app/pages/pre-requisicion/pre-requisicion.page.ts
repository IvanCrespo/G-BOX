...
export class PreRequisicionPage implements OnInit {
  ...
  constructor(
    ...
    var d = new Date();
    this.d_fecha = ("00" + d.getDate()).slice(-2) + "-" + ("00" + (d.getMonth() + 1)).slice(-2) + "-" + d.getFullYear();
    this.t_hora = ("00" + d.getHours()).slice(-2) + ":" + ("00" + d.getMinutes()).slice(-2) + ":" + ("00" + d.getSeconds()).slice(-2);
    this.getGeolocation();
  ...
  save() {
    ...
    if (this.datos.s_nota_pre_requisicion == '' || this.datos.s_nota_pre_requisicion == undefined) {
      this.presentToast("Campo Nota vacio");
    }
    else {
      this.inventarioServ.Post(this.token, this.url, this.datos).subscribe(
        (data: any) => {
          console.log(data);
          if (data.status == 'fail') {
